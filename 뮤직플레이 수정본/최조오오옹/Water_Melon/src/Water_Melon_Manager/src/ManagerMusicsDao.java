...
public interface ManagerMusicsDao {
	// C : 음원 생성(추가)
	...
	// U : 음원 정보(텍스트) 수정
	int update(Connection conn, String title, String artist, String genre, InputStream mp3, int mp3Length, int music_id) throws SQLException;
	
	// D : 음원 삭제
	int delete(Connection conn, int music_id) throws SQLException;
	
	int lastRank(Connection conn) throws SQLException;
}
