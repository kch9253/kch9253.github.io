...
public class EditDialog extends JDialog implements ActionListener {
	...
	public EditDialog(JDialog owner) {
		super(owner,true);
		setModal(true);
	...
	public  void actionPerformed(ActionEvent e ) {
		...
		if (e.getSource() == btn2) {
			...
			AdminFrame owner = (AdminFrame) getOwner();
			int music_id = owner.getCheckedIds().get(0);
			...
			if (title == null || artist == null || genre == null || musicFis == null) {
				...
				try {
					conn = DBUtil.getConnection();
					 ...
					 dao.update(conn, title, artist, genre, musicFis, musicLength, music_id);
					JOptionPane.showMessageDialog(null, "수정이 완료되었습니다.", "확인", JOptionPane.DEFAULT_OPTION);
					dispose();
