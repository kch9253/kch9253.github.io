...
public class StreamingMain extends JFrame {
	...
	public StreamingMain(String user_id) throws SQLException {
		...
		URL url = getClass().getClassLoader().getResource("img/리스트마이페이지.png");
		URL ur2 = getClass().getClassLoader().getResource("img/로그아웃.png");
		URL ur3 = getClass().getClassLoader().getResource("img/플레이리스트.png");
		URL ur4 = getClass().getClassLoader().getResource("img/스마일.png");
		URL ur5 = getClass().getClassLoader().getResource("img/플레이리스트담기.png");
		URL ur6 = getClass().getClassLoader().getResource("img/플레이리스트전부.png");
		URL ur7 = getClass().getClassLoader().getResource("img/광고메인.jpg");
		...
		JButton MyPage = new JButton(new ImageIcon(url));
		...
		JButton logoutBtn = new JButton(new ImageIcon(ur2));
		sl_panel.putConstraint(SpringLayout.NORTH, logoutBtn, 0, SpringLayout.NORTH, MyPage);
		sl_panel.putConstraint(SpringLayout.WEST, logoutBtn, 344, SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.SOUTH, logoutBtn, 0, SpringLayout.SOUTH, MyPage);
		sl_panel.putConstraint(SpringLayout.EAST, logoutBtn, -32, SpringLayout.EAST, panel);
		...
		JButton playlistBtn = new JButton(new ImageIcon(ur3));
		sl_panel.putConstraint(SpringLayout.WEST, playlistBtn, 189, SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.EAST, playlistBtn, -40, SpringLayout.WEST, logoutBtn);
		sl_panel.putConstraint(SpringLayout.EAST, MyPage, -31, SpringLayout.WEST, playlistBtn);
		sl_panel.putConstraint(SpringLayout.NORTH, playlistBtn, -5, SpringLayout.NORTH, MyPage);
		sl_panel.putConstraint(SpringLayout.SOUTH, playlistBtn, 0, SpringLayout.SOUTH, MyPage);
		playlistBtn.setBorderPainted(false); // 버튼외곽선 없애줌
		playlistBtn.setContentAreaFilled(false); // 버튼 투명하게설정
		panel.add(playlistBtn);
		JLabel lblNewLabel_2 = new JLabel(PLdao.findUsersName(user_id) + "님 환영합니다.");
		sl_panel.putConstraint(SpringLayout.NORTH, MyPage, 47, SpringLayout.SOUTH, lblNewLabel_2);
		sl_panel.putConstraint(SpringLayout.EAST, lblNewLabel_2, -24, SpringLayout.EAST, MyPage);
		sl_panel.putConstraint(SpringLayout.WEST, lblNewLabel_2, 0, SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.SOUTH, lblNewLabel_2, -110, SpringLayout.SOUTH, panel);
		...
		sl_panel.putConstraint(SpringLayout.WEST, lblNewLabel_5, 334, SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.EAST, lblNewLabel_5, -42, SpringLayout.EAST, panel);
		...
		JLabel lblNewLabel_6 = new JLabel(new ImageIcon(ur4));
		sl_panel.putConstraint(SpringLayout.NORTH, lblNewLabel_6, 10, SpringLayout.NORTH, panel);
		sl_panel.putConstraint(SpringLayout.WEST, lblNewLabel_6, 0, SpringLayout.EAST, lblNewLabel_2);
		sl_panel.putConstraint(SpringLayout.SOUTH, lblNewLabel_6, 0, SpringLayout.SOUTH, lblNewLabel_2);
		sl_panel.putConstraint(SpringLayout.EAST, lblNewLabel_6, -322, SpringLayout.EAST, panel);
		...
		sl_rank_panel.putConstraint(SpringLayout.SOUTH, rankPanel9, -110, SpringLayout.SOUTH, rank_panel);
		...
		JButton btnNewButton = new JButton(new ImageIcon(ur5));
		sl_rank_panel.putConstraint(SpringLayout.NORTH, btnNewButton, 383, SpringLayout.NORTH, rank_panel);
		sl_rank_panel.putConstraint(SpringLayout.SOUTH, btnNewButton, -25, SpringLayout.SOUTH, rank_panel);
		sl_rank_panel.putConstraint(SpringLayout.SOUTH, panel_2, -25, SpringLayout.NORTH, btnNewButton);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (list.size() == 0) {
					...
					for (int i = 0; i < list.size(); i++) { // 플레이리스트 테이블에 데이터 입력
						try {
							PLdao.SetSaveCnt(list.get(i));
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
						try {
							PLdao.insertPL(user_id, list.get(i));
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
					}
		...
		JButton btnNewButton_1 = new JButton(new ImageIcon(ur6));
		sl_rank_panel.putConstraint(SpringLayout.NORTH, btnNewButton_1, 383, SpringLayout.NORTH, rank_panel);
		...
		btnNewButton_1.addActionListener(new ActionListener() { // 창희추가
			@Override
			public void actionPerformed(ActionEvent e) {
				MenuJTabaleExam_ch dialog;
				dialog = new MenuJTabaleExam_ch(StreamingMain.this, user_id, rankCheck);
				dialog.show();
			}
		});
		...
		sl_rank_panel.putConstraint(SpringLayout.NORTH, lblNewLabel_7, 0, SpringLayout.NORTH, lblNewLabel_8);
		sl_rank_panel.putConstraint(SpringLayout.NORTH, lblNewLabel_8, 413, SpringLayout.NORTH, rank_panel);
		sl_rank_panel.putConstraint(SpringLayout.SOUTH, lblNewLabel_7, 0, SpringLayout.SOUTH, lblNewLabel_8);
		sl_rank_panel.putConstraint(SpringLayout.SOUTH, lblNewLabel_8, 0, SpringLayout.SOUTH, rank_panel);
		...
		JLabel lblNewLabel_1 = new JLabel(new ImageIcon(ur7));
