...
public ResultSet r; 	
	...
	private JCheckBox c1 = new JCheckBox("Name", true);	// ok 1
	private JCheckBox c2 = new JCheckBox("Ssn", true); // ok 2
	private JCheckBox c3 = new JCheckBox("Bdate", true); 
	private JCheckBox c4 = new JCheckBox("Address", true); // ok  4
	private JCheckBox c5 = new JCheckBox("Sex", true);	// ok 5
	private JCheckBox c6 = new JCheckBox("Salary", true);	// ok 6
	private JCheckBox c7 = new JCheckBox("Supervisor", true); // ok 7
	private JCheckBox c8 = new JCheckBox("Department", true); // ok 8	
	...
	setVisible(true);
		...
		if (e.getSource() == Search_Button) {			
			...
			if (c6.isSelected()) {					
				...
				if (c8.isSelected()) {					
					if (!c1.isSelected() && !c2.isSelected() && !c3.isSelected() && !c4.isSelected() && !c5.isSelected()							&& !c6.isSelected() && !c7.isSelected())
						stmt += " dname";
					else						
						stmt += ", dname";
					Head.add("DEPARTMENT");				
		...
		if (e.getSource() == Delete_Button) {		
			...
			try { 				
				...
				if (columnName == "SSN") {					
					...
					} else {JOptionPane.showMessageDialog(null, "삭제 작업을 진행하시려면 NAME, SSN 항목을 모두 체크해주세요.");
					} 	ShowSelectedEmp.setText(" "); 
					} catch (SQLException e1) {				
